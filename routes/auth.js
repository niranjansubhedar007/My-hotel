...
router.patch('/admin/:id/direct', async (req, res) => {
    ...
    try {
        ...
        // Check if admin is an instance of Admin or AdminBar
        ...
        } else if (admin instanceof AdminBar) {
            admin = await AdminBar.findByIdAndUpdate(
                id,
                { isDirect: newIsDirectValue },
                { new: true }
        ...
        // If admin is null after update, return an error
        if (!admin) {
...
router.patch('/admin/:id/lan', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        // Toggle the value of isLan
        const newIsLanValue = !admin.isLan;

        // Update the record with the new isLan value
        if (admin.modelName === 'Admin') {
            admin = await Admin.findByIdAndUpdate(
                id,
                { isLan: newIsLanValue },
...
router.patch('/admin/:id/lanbill', async (req, res) => {
    const { id } = req.params;
    ...
    try {
        ...
        // Toggle the value of lanBill
        const newLanBillValue = !admin.islanBill;

        // Update the record with the new lanBill value
        if (admin.modelName === 'Admin') {
            admin = await Admin.findByIdAndUpdate(
                id,
