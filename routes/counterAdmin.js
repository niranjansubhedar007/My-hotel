...
const COUNTERADMINA_JWT_SECRET = "counterAdmin123456@admin123456";
...
router.post("/setup", async (req, res) => {
  ...
  try {
    ...
    const existingAdmin = await CounterAdmin.findOne({
      $or: [{ username }, { email }],
    });
    if (existingAdmin) {
      return res
    ...
    const newAdmin = new CounterAdmin({
      username,
      email,
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const admin = await CounterAdmin.findOne({ username });
      ...
      if (passwordMatch) {
        const token = jwt.sign(
          { username: admin.username, role: "counterAdmin" },
          COUNTERADMINA_JWT_SECRET
        );
...
// PATCH route to toggle the islanBill field for CounterAdmin
router.patch('/counteradmin/:id/lanbill', async (req, res) => {
  const { id } = req.params;

  try {
      // Find the CounterAdmin by ID
      let counterAdmin = await CounterAdmin.findById(id);

      // If not found, return a 404 error
      if (!counterAdmin) {
          return res.status(404).json({ message: 'CounterAdmin not found' });
      }

      // Toggle the value of islanBill
      const newLanBillValue = !counterAdmin.islanBill;

      // Update the record with the new islanBill value
      counterAdmin = await CounterAdmin.findByIdAndUpdate(
          id,
          { islanBill: newLanBillValue },
          { new: true }
      );

      res.status(200).json({ message: 'CounterAdmin updated successfully', counterAdmin });
  } catch (error) {
      console.error(error);
...
// GET route to fetch all CounterAdmin records
router.get('/counteradmins', async (req, res) => {
  try {
      // Find all CounterAdmin records
      const counterAdmins = await CounterAdmin.find();

      // If no records are found, return an empty array
      res.status(200).json({ message: 'CounterAdmins fetched successfully', counterAdmins });
  } catch (error) {
      console.error(error);
      res.status(500).json({ message: 'Server error' });
  }
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const admin = await CounterAdmin.findOne({
        resetToken,
