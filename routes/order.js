...
router.patch('/order/update-order-by-table/:tableId', async (req, res) => {
  try {
    const { tableId } = req.params;
    const { items = [], subtotal, CGST, SGST, total, menuTotal, isTemporary, isPrint, VAT, barSubtotal, grandTotal, acPercentageAmount, acPercentage, vatPercentage, gstPercentage, lastTotal, waiterName,sectionName } = req.body;
...
router.patch('/update-order-by-id/:orderId', async (req, res) => {
  try {
    const { orderId } = req.params;
    const { items = [],customerName,mobileNumber, subtotal, VAT, CGST, SGST, total, isTemporary, acPercentageAmount, isPrint, cashAmount, onlinePaymentAmount, dueAmount, complimentaryAmount, discount, barSubtotal, menuTotal, grandTotal, acPercentage, vatPercentage, gstPercentage, lastTotal, waiterName,sectionName } = req.body;
...
router.patch('/mergeTables', async (req, res) => {
  try {
    ...
    // Merge acPercentage
    destinationOrder.acPercentageAmount += sourceOrder.acPercentageAmount;
