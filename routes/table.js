...
router.delete('/tables/:tableId', async (req, res) => {
  ...
  router.delete('/tables', async (req, res) => {
    try {
      ...
      if (!ids || ids.length === 0) {
        return res.status(400).json({ error: 'No table IDs provided for deletion' });
      }
  
      // Find all tables to delete
      const tabletodelete = await Table.find({ _id: { $in: ids } });
  
      // Check if the tables exist
      if (tabletodelete.length === 0) {
        return res.status(404).json({ error: 'No tables found for deletion' });
      }
  
      // Get IDs of the splitTables that reference the tables to be deleted
      const splitTableIds = tabletodelete.flatMap(table => 
        table.splitTables.map(splitTable => splitTable.tableId)
      );
  
      // Delete the associated splitTables
      await Table.deleteMany({ parentTable: { $in: ids } });
  
      // Now delete the main tables
      const result = await Table.deleteMany({ _id: { $in: ids } });
  
      res.status(200).json({
        message: 'Tables and their associated splitTables deleted successfully',
        deletedCount: result.deletedCount
      });
    } catch (error) {
      console.error('Error deleting tables and their splitTables:', error);
...
router.put('/:sectionId/:mainTableId/show-subtables', async (req, res) => {
    ...
    try {
        ...
        if (subtables.length < numberOfSubtablesToShow) {
            return res.status(400).json({ message: 'Not enough available subtables to show' });
        }
        ...
        // No need to update mainTable.isCalled, keeping the main table visible
        res.status(200).json({ message: `Successfully updated isCalled property for ${numberOfSubtablesToShow} subtables, along with main table.` });
    } catch (err) {
        res.status(400).json({ message: err.message });
    }
});
